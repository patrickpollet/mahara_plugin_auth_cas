...
class php4DOMNode {
    ...
    function add_child ($newnode) {
        return $this->append_child ($newnode);
    }
...
class php4DOMXPath {
    ...
    function xpath_eval ($eval_str, $contextnode = null) {
        if (method_exists ($this->myDOMXPath, 'evaluate')) {
            $xp = isset($contextnode->myDOMNode) ? $this->myDOMXPath->evaluate ($eval_str, $contextnode->myDOMNode) : $this->myDOMXPath->evaluate ($eval_str);
        }
        else {
            $xp = isset($contextnode->myDOMNode) ? $this->myDOMXPath->query ($eval_str, $contextnode->myDOMNode) : $this->myDOMXPath->query ($eval_str);
        }
...
if (extension_loaded ('xsl')) { //See also: http://alexandre.alapetite.fr/doc-alex/xslt-php4-php5/
