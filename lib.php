...
/**
 * Authenticates users with CAS and an associtaed Lightweight Directory Access Protocol
 */
class AuthCas extends AuthLdap
{
    ...
    public function __construct($id = null)
    {
        ...
        $this->type = 'cas';
        $this->has_instance_config = true; //temporary out
        $this->config['studentidfield2'] = 'supannEmpId';
    ...
    public function authenticate_user_account($user, $password)
    {
        ...
        // this may happen if CAS user typed its credentials in the Mahara login box ...
        ...
        /*******************
        NO NO
        la session a été détruite par mahara (dans login_submit) donc isAuthenticated echoue ici
        $this->connectCAS();
        ...
        */
        ...
        if ($user->id && $update) {
            ...
            $ldapdetails = $this->get_user_info($username, $ldapattributes);
    ...
    /**
     * @override
     * logout from CAS is specified in the configuration
     */
